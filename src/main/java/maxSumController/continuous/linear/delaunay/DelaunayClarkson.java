...
/*
 * The Delaunay triangulation algorithm in this class is originally from hull by
 * Ken Clarkson:
 * 
 ...
 * hereby granted, provided that this entire notice is included in all copies of
 * any software which is or includes a copy or modification of this software and
 * in all copies of the supporting documentation for such software. THIS
...
/**
 * DelaunayClarkson represents an O(N*logN) method with high overhead to find
 * the Delaunay triangulation of a set of samples of R^DomainDimension.
 * <P>
 */
public class DelaunayClarkson extends Delaunay {
	...
	// <<<< Variables >>>>
	/*
	 * we need to have two indices for every pointer into basis_s and simplex
	 * arrays, because they are two-dimensional arrays of blocks. ( _bn = block
	 * number )
	 */
	...
	/**
	 * starting at s, visit simplices t such that test(s,i,0) is true, and t is
	 * the i'th neighbor of s; apply visit function to all visited simplices;
	 * when visit returns nonnull, exit and return its value.
	 */
	private void visit_triang_gen(int s, int s_bn, int whichfunc, int[] ret,
			int[] ret_bn) {
		int v;
		int v_bn;
		int t;
	...
	/**
	 * visit simplices s with sees(p,s), and make a facet for every neighbor of
	 * s not seen by p.
	 */
	...
	private void extend_simplices(int s, int s_bn, int[] ret, int[] ret_bn) {
		...
		if (sbt_peak_vert[s_bn][s] == NOVAL) {
			...
			extend_simplices(sbt_peak_simp[s_bn][s], sbt_peak_simp_bn[s_bn][s],
					esretp, esretp_bn);
	...
	/**
	 * return a simplex s that corresponds to a facet of the current hull, and
	 * sees(p, s).
	 */
	 ...
	 */
	public DelaunayClarkson(double[][] samples) {
		int s, s_bn, q, q_bn;
		int root, root_bn;
		// int k = 0;
		int[] retp = new int[1];
		...
		if (nrs <= dim)
			throw new IllegalArgumentException("DelaunayClarkson: "
					+ "not enough samples");
		if (dim > MAXDIM)
			throw new IllegalArgumentException("DelaunayClarkson: "
		 ...
		 * for (int j=0; j<dim; j++) { for (int kk=0; kk<nrs; kk++) {
		...
		if (rdim > MAXDIM)
			throw new IllegalArgumentException(
					"dimension bound MAXDIM exceeded; rdim=" + rdim + "; dim="
							+ dim);
