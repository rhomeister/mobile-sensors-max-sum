...
//  |z| < cutoff = 10/sqrt(2).
   ...
   //This method quickly generates samples from independent N(0,1) random variables.
	public static Matrix genSamples(int n, int samples, Random rand) {
		Matrix result = new Matrix(n, samples);
		for(int i=0; i<n; i++) {
			int j;
			for(j = (samples-1); j > 0; j-=2) {
				double r = - Math.log(rand.nextDouble());
				double theta = 2 * Math.PI * rand.nextDouble();
				result.set(i, j, r * Math.sin(theta));
				result.set(i, j-1, r * Math.cos(theta));
			}
			if(j==0) {
				double r = - Math.log(rand.nextDouble());
				double theta = 2 * Math.PI * rand.nextDouble();
	...
	// For non-normalised distributions
	public static double N(double y, double m, double sd){
		return (1/sd)*Math.exp((-0.5)*(y-m)*(y-m)/(sd*sd));
		}
	
	public static double phi(double y, double m, double sd){
		return normp((y-m)/sd);
		}
