...
/**
 * DelaunayCustom is a set of constructors to create an instance of Delaunay by
 * passing in a pre-computed triangulation. DelaunayCustom is useful for
 * creating instances of Delaunay that can be passed into IrregularSet. If you
 * want to perform consistency checks on your triangulation, call
 * Delaunay.test() on your DelaunayCustom object after it is constructed.
 * <P>
 */
public class DelaunayCustom extends Delaunay {

	// /**
	// * construct a Delaunay from point locations and a list of triangles; call
	// * finish_triang() to fill in helper arrays (vertices, walk and edges);
	// copy
	// * arguments
	// *
	// * @param samples
	...
	// * @param tri
	// * list of triangles - dimensioned int[ntris][dim + 1] tris
	// * values are indices into second index of samples
	...
	//
	// /**
	// * construct a Delaunay from point locations, a list of triangles, and
	// * helper arrays (vertices, walk and edges); copy arguments
	// *
	// * @param samples
	// * locations of points for topology - dimensioned
	// * float[dimension][number_of_points]
	...
	// * values are indices into second index of samples
	// * @param vertices
	// * links from vertices to triangles/tetrahedra - dimensioned
	// * int[number_of_points][nverts[i]]
	// * @param walk
	...
	// */
	// public DelaunayCustom(float[][] samples, int[][] tri, int[][] vertices,
	// int[][] walk, int[][] edges, int num_edges, boolean copy) {
	// if (tri == null) {
	// throw new IllegalArgumentException("DelaunayCustom: "
	// + "Tri array must be specified!");
	// }
	// if (samples == null && vertices == null) {
	// throw new IllegalArgumentException(
	// "DelaunayCustom: Cannot construct "
	...
	// System.arraycopy(vertices[i], 0, Vertices[i], 0,
	// vertices[i].length);
	// }
	// }
	// if (walk != null) {
	...
	// throw new IllegalArgumentException(
	// "Gridded2DSet musy have manifold dimension = 1");
	// }
	...
	// throw new IllegalArgumentException(
	// "samples argument bad dimensions");
	// }
	...
	//
	// /**
	// * determine if a closed path self-intersects, and remove consecutive
	// * identical points
	// *
	// * @param samples
	// * locations of points on closed path - dimensioned
	// * float[2][number_of_points] - may be modified on return
	// * @return true if closed path in samples self-intersects
	// * @throws VisADException
	// * a VisAD error occurred
	// */
	...
	// if (x == 0.0f) {
	// samples[0][i] = SELF * a0 + PULL2
	// * (b0 + samples[0][prev[i]]);
	// samples[1][i] = SELF * a1 + PULL2
	// * (b1 + samples[1][prev[i]]);
	// } else if (x == 1.0f) {
	// k = next[i];
	// samples[0][k] = SELF * b0 + PULL2
	// * (a0 + samples[0][next[k]]);
	// samples[1][k] = SELF * b1 + PULL2
	// * (a1 + samples[1][next[k]]);
	// }
	// if (y == 0.0f) {
	// samples[0][j] = SELF * c0 + PULL2
	// * (d0 + samples[0][prev[j]]);
	// samples[1][j] = SELF * c1 + PULL2
	// * (d1 + samples[1][prev[j]]);
	// } else if (y == 1.0f) {
	// k = next[j];
	// samples[0][k] = SELF * d0 + PULL2
	// * (c0 + samples[0][next[k]]);
	// samples[1][k] = SELF * d1 + PULL2
	// * (c1 + samples[1][next[k]]);
	// }
	// }
	// }
	...
	// * @throws VisADException
	// * path self intersects or a VisAD error occurred
	// */
	...
	// if (set.getManifoldDimension() != 1) {
	// throw new IllegalArgumentException(
	// "UnionSet must have manifold dimension = 1");
	// }
	...
	// throw new IllegalArgumentException(
	// "UnionSet must contain only Gridded2DSets");
	// }
	...
	// *
	// * @param set
	// * Gridded2DSet with manifold dimension = 1, interpreted as a
	// * closed path
	...
	// throw new IllegalArgumentException("path self intersects");
	// }
	// int n = samples[0].length;
	//
	// // build circular boundary list
	...
	// float area = 0.0f;
	// for (int i = 0; i < n; i++) {
	// area += samples[0][i] * samples[1][next[i]] - samples[0][next[i]]
	// * samples[1][i];
	// }
	...
	// region;
	// * return a decomposition of that region into triangles whose vertices are
	// * all boundary points from samples, as an Irregular2DSet
	// *
	// * @param set
	// * Gridded2DSet with manifold dimension = 1, interpreted as a
	// * closed path boundary
	// * @return Irregular2DSet whose triangles form the interior of the region
	// * enclosed by set
	// * @throws VisADException
	// * path self intersects or a VisAD error occurred
	// */
	...
	// * closed path boundary
	// * @param check
	// * if true then throw a VisADException if path self- intersects,
	...
	// if (delaunay == null)
	// return null;
	// return new IllegalArgumentException(set.getType(), samples, null, null,
	// null, delaunay);
	// }
	//
	// /**
	// * check that samples describes the boundary of a simply connected plane
	// * region; return a decomposition of that region into triangles whose
	// * vertices are all boundary points from samples; the trick is that the
	// * region may not be convex, but the triangles must all lie inside the
	// * region
	// *
	// * @param samples
	// * locations of points on closed path - dimensioned
	...
	// // if (bug && !in) System.out.println("bug " + i + " tri orient in =
	// // " + in);
	// if (in && i != next[k]) {
	...
	// in = false;
	// }
	// // if (bug && !in) System.out.println("bug " + i + " adjacent in
	// // = " + in);
	// if (in) {
	// int p = next[k];
	// int q = next[p];
	...
	// if (Math.abs(det) == 0.0)
	// continue;
	// float x = ((b0 - a0) * (c1 - a1) - (b1 - a1)
	// * (c0 - a0))
	// / det;
	// float y = ((c0 - a0) * (c1 - d1) - (c1 - a1)
	// * (c0 - d0))
	// / det;
	// if (0.0f <= x && x <= 1.0f && 0.0f <= y && y <= 1.0f) {
	// in = false;
	...
	// }
	// } // end while (q != i)
	// // if (bug && !in) System.out.println("bug " + i + "
	// // intersect in = " + in);
	// } // end if (in)
	// } // end if (in && i != next[k])
	// if (in) {
	...
	// // System.out.println("bad samples t = " + t + " n = " +
	// // n);
	// if (t > 0) {
	...
	// if (delaunay == null)
	// return null;
	// return new Irregular2DSet(set.getType(), samples, null, null, null,
	// delaunay);
	// }
	//
	// /**
	// * link multiple paths into a single path; this assumes that the paths in
	// ss
	// * don't intersect each other but does test for self-intersection by each
	// * path
	// *
	// * @param ss
	...
	// * @throws VisADException
	// * a path self intersects or a VisAD error occurred
	// */
	...
	//
	// /**
	// * clip the topology (samples, tris) against the half-plane xc * x + yc *
	// y <=
	// * v and return the clipped topology
	// *
	// * @param samples
	// * locations of points for topology - dimensioned
	// * float[dimension][number_of_points]
	...
	// * values are indices into second index of samples
	// * @param xc
	// * x coefficient in half-plane inequality
	// * @param yc
	// * y coefficient in half-plane inequality
	// * @param v
	// * constant in half-plane inequality
	// * @param outs
	// * array dimensioned float[1][][] to take samples of clipped
	// * topology - on output dimensioned
	// * float[1][2][number_of_output_samples]
	// * @param outt
	// * array dimensioned int[1][][] to take tris of clipped topology -
	// * on output dimensioned int[1][number_of_output_triangles][3]
	// * @throws VisADException
	// * a VisAD error occurred
	// */
	// public static void clip(float[][] samples, int[][] tris, float xc,
	// float yc, float v, float[][][] outs, int[][][] outt) {
	// float[][] s = null;
	// int[][] t = null;
	...
	// break;
	// case 3:
	// t[nt] = new int[] { smap[tris[i][0]], smap[tris[i][1]],
	// smap[tris[i][2]] };
	// nt++;
	// break;
	// case 1:
	...
	// }
	// /*
	// * if (ss != null) { int nn = samples[0].length; for (int ii=0; ii<nn;
	// * ii++) { System.out.println("samples[][" + ii + "] = " +
	// * samples[0][ii] + " " + samples[1][ii]); } if (tris != null) { nn =
	// * tris.length; for (int ii=0; ii<nn; ii++) {
	// * System.out.println("tris[" + ii + "] = " + tris[ii][0] + " " +
	// * tris[ii][1] + " " + tris[ii][2]); } } System.out.println("nskept = " +
	// * nskept + " ns = " + ns + " nt = " + nt); nn = ss[0].length; for (int
	// * ii=0; ii<nn; ii++) { System.out.println("ss[][" + ii + "] = " +
	// * ss[0][ii] + " " + ss[1][ii]); } if (tt != null) { nn = tt.length; for
	// * (int ii=0; ii<nn; ii++) { System.out.println("tt[" + ii + "] = " +
	// * tt[ii][0] + " " + tt[ii][1] + " " + tt[ii][2]); } } for (int ii=0;
	// ii<smap.length;
	// * ii++) { System.out.println("smap[" + ii + "] = " + smap[ii]); } for
	// * (int ii=0; ii<sother.length; ii++) { System.out.println("sother[" +
	// * ii + "] = " + sother[ii][0] + " " + sother[ii][1] + " " +
	// * sother[ii][2] + " " + sother[ii][3] + " " + sother[ii][4] + " " +
	// * sother[ii][5]); System.out.println("snew[" + ii + "] = " +
	// * snew[ii][0] + " " + snew[ii][1] + " " + snew[ii][2] + " " +
	// * snew[ii][3] + " " + snew[ii][4] + " " + snew[ii][5]); } } else {
