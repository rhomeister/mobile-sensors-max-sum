...
public abstract class Delaunay implements java.io.Serializable {
	...
	public static Delaunay factory(double[][] samples, boolean exact) {
		...
		if (dim < 2)
			throw new IllegalArgumentException("Delaunay.factory: "
	 ...
	 */
	// public static float[][] scale(float[][] samples, float mult, boolean
	// copy) {
	// int dim = samples.length;
	// int nrs = samples[0].length;
	// for (int i = 1; i < dim; i++) {
	...
	public void improve(double[][] samples, int pass) {
		...
		if (Tri[0].length != dim1) {
			throw new IllegalArgumentException(
					"Delaunay.improve: samples dimension " + "does not match");
		}
		// only 2-D triangulations supported for now
		if (dim > 2) {
			throw new NotImplementedException("Delaunay.improve: dimension "
					+ "must be 2!");
		}
		int ntris = Tri.length;
		int nrs = samples[0].length;
